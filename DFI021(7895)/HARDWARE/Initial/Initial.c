...
static void EXTINT_Config(void)
{
	/*
	(1)设置EXTINT功能
	*/
...
void	InitSys(void)
{
	...
	// CLKCON =Bin(00100000);
	// SYS_ConfigSystemClk(SYS_CLK_HSI);			//选择外部高速晶振
	// SYS_SET_SYSTEM_CLK(2);
	...
	P3TRIS = (P3TRIS & ~((1 << 0))) | (0);
	GPIO_ENABLE_UP(P1UP, GPIO_PIN_1);				//开启下拉
	GPIO_ENABLE_UP(P1UP, GPIO_PIN_2);				//开启下拉
	// P0TRIS = Bin(10001110); 	//方向控制，1：输出，0：输入，
	// P0DS = 0xff;				//P00输出输入均读取引脚状态
	// P1DS = 0xff;				//P00输出输入均读取引脚状态
	// P2DS = 0xff;				//P00输出输入均读取引脚状态
	// P3DS = 0xff;				//P00输出输入均读取引脚状态
	...
	GPIO_SET_MUX_MODE(P05CFG, GPIO_P05_MUX_AN5);
