...
//长按，连加选择			         POW   FLA  HEA  TIM  DWL   TEST
//uint8 code ShortPressList[ ]={ 0  , 0  ,  0 ,  0 ,  1 ,  1 ,  1 };
//uint8 code LinkPressList [ ]={ 0  , 0  ,  0 ,  0 ,  0 ,  0 ,   1 ,  0 };
uint8 code LongPressList [ ]={ LNC,   0  , LNC, 100, 100,  0,  LNC, LNC, LNC};
//-----------------------
/************************************************************
* 函数名: UpKey()
* 功能说明:  按键调用
...
void KeyCall(unsigned char KeyData)
{
  ...
  switch(KeyData){
	...
	//-----------------------------
	case 0x05:HeaKey()        ;break;
	case 0x06:HeatKey(1)      ;break;
#if (DWL_EN==1)
    case 0x07:DwlKey()        ;break;
#endif
    //-----------------------------
	case 0x08:TempProcKey()   ;break;
	//-----------------------------
    case   21:SysRst()        ;break;
    case   23:HeaLockSw()     ;break;
	case   24:GearsWidthKey() ;break;
    //-----------------------------
#if (DWL_EN==1)
